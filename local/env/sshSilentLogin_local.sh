...
## 配置文件目录
export CONF_DIR=${ROOT_HOME}/local/conf
##扩展集群配置文件目录
...
## 系统root 用密码
export PASSWORD=$(grep SSH_Password ${CONF_DIR}/local_conf.properties|cut -d '=' -f2)
## authorized_keys 内容所在文件
export AUTHORIZED_KEYS=${BIN_DIR}/authorized_keys.log

## 集群所有节点主机名，放入数组中
CLUSTER_HOSTNAME_ARRY=$(grep ES_InstallNode ${CONF_DIR}/local_conf.properties|cut -d '=' -f2)
