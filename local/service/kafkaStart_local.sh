...
## kafka 的安装节点，放入数组中
KAFKA_HOSTNAME_LISTS=$(grep Kafka_InstallNode ${CONF_DIR}/local_conf.properties|cut -d '=' -f2)
## KAFKA_HOME  kafka 根目录
KAFKA_HOME=${INSTALL_HOME}/Kafka/kafka

## 集群组件的日志文件目录:
CLUSTER_LOGSDIR=$(grep Cluster_LOGSDir ${CONF_DIR}/local_conf.properties|cut -d '=' -f2)
