...
## rocketmq 集群节点
ROCKETMQ_HOST=$(grep RocketMQ_Namesrv ${CONF_DIR}/local_conf.properties|cut -d '=' -f2)
## rocketmq 安装目录
...
##rocketMq 日志目录
ROCKETMQ_LOG=$(grep Cluster_LOGSDir ${CONF_DIR}/local_conf.properties|cut -d '=' -f2)
...
RocketMQWebUI_Dir=$(grep WebUI_Dir ${CONF_DIR}/local_conf.properties|cut -d '=' -f2)
