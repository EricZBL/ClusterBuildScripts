...
{
   ## 在collect中配置ftpip
   echo "配置collect.properties里的ftpip"
   if [[ -n ${FTPIP} ]]; then
       sed -i "s#ftp.ip=.*#ftp.ip=${FTPIP}#g" ${COllECT_CONF_FILE}
       else
       local=$(grep `hostname` /etc/hosts | awk '{print $1}')
       if [[ ! -n $local ]]; then
           echo "`hostname` /etc/hosts 中的配置有问题，请检查"
           exit 1
       fi
   fi
...
function distribute_collect()
{
   ...
   for n in ${num[@]};do
       ...
       for ip in ${ftplist[@]}; do
           ...
           ssh root@${ip} "sed -i 's#^ftp.type=.*#ftp.type=${type}#g' ${COllECT_CONF_FILE}"
           ##修改配置文件中的proxy ip
           echo "${ip}上分发collect完毕，开始配置配置文件中proxy.ip................." | tee -a $LOG_FILE
           ssh root@${ip} "sed -i 's#^proxy.ip=.*#proxy.ip=${haproxy}#g' ${COllECT_CONF_FILE}"
           ##修改配置文件中的ftp ip
