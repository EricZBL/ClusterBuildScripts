...
##在主配置文件中追加
    if [[ "$component" = "datanode" ]] ; then
        for node in ${EXPEND_NODE_ARRY[@]}; do
    ...
    if [[ "$component" = "nodemanager" ]] ; then
        ...
        NUM=$(grep Yarn_NumOfNodeManger ${ROOT_HOME}/conf/cluster_conf.properties |cut -d '=' -f2)
        EXPEND_NUM=${#EXPEND_NODE_ARRY[@]}
        sed -i "s#Yarn_NumOfNodeManger=$NUM#Yarn_NumOfNodeManger=$[$NUM+$EXPEND_NUM]#g" ${ROOT_HOME}/conf/cluster_conf.properties
        echo "扩展后nodemanager为 $[$NUM+$EXPEND_NUM] 个"
    fi
